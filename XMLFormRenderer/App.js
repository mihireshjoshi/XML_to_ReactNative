...
const App = () => {
  ...
  const parseXML = () => {
    parseString(xmlData, (err, result) => {
      ...
      const elements = [];
      const formSideMain =
        result.div.div[0].svg[0].svg[0].g.find(
          (g) => g.$?.class === "formSideMain"
        ) || {};

      const groups = formSideMain.g || [];
      const texts = formSideMain.text || [];

      // Map each `text` label to the previous `g` containing `rect` elements
      groups.forEach((group, index) => {
        const rects = group.rect || [];
        if (index < texts.length) {
          const label = texts[index]?._ || "";
          const inputs = rects.map((rect, idx) => ({
            id: `${label}-${idx}`,
            x: rect.$.x,
            y: rect.$.y,
            width: rect.$.width,
  ...
  const renderForm = () => {
    return formElements.map((group, index) => (
      <View key={index} style={styles.group}>
        {group.label && <Text style={styles.label}>{group.label}</Text>}
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>Dynamic Form</Text>
      {renderForm()}
      <Text style={styles.button} onPress={parseXML}>
        Parse XML
      </Text>
    </ScrollView>
  );
